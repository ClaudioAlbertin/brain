var Algorithm       = require('../algorithm')
  ...
  , BackPropagation = require('./back-propagation');
...
GradientDescent.prototype.clone = function () {
  return Algorithm.clone(GradientDescent, this);
};
...
GradientDescent.prototype.run = function () {
  ...
  for (i = 0, l = this.options.iterations; i < this.options.iterations; i++) {
    ...
    if (i % this.options.reportFrequency === 0) {
      ...
      speed = (lastError - error) / this.options.reportFrequency;
      ...
      if (isNaN(error) || error <= this.options.targetError || speed < this.options.minimalSpeed) break;
