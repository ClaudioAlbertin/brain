...
exports.randomWeights = function (layers, bias) {
  ...
  for (var i = 1, l = layers.length, layer, previousLayer; i < l; i++) {
    layer         = layers[i];
...
exports.zeroWeights = function (layers, bias) {
  ...
  for (var i = 1, l = layers.length, layer, previousLayer; i < l; i++) {
    ...
    // generate weights for the given number of units and an additional bias unit
    weights.push(Matrix.Zero(layer, previousLayer + ((bias === false) ? 0 : 1)));
  }
...
/**
 * Converts a multidimensional array to an array of matrices that can be used as examples
...
exports.normalize = function (value, scale) {
  return (value - scale.avg) / scale.abs;
};
...
exports.denormalize = function (value, scale) {
  return (value * scale.abs) + scale.avg;
};
...
exports.addBiasUnit = function (vector, modifier) {
  var unit = (modifier) ? modifier(1) : 1;

  var biasedVector = vector.elements.slice(0);
...
/**
 * Write JSON file with network configuration
 * @param  {String}   path     File path
 * @param  {Object}   setup    Setup to write
 * @param  {Function} callback Callback
 *
 * (1) [Number]
 * (2) [Matrix]
 */
exports.writeJSON = function (path, setup, callback) {
  fs.writeFile(path, JSON.stringify(setup), callback);
};
