...
AnalysisRunner.prototype.toData = function () {
  ...
  data = {
    convergence : utils.generateDataLegend(['iterations', 'convergence']),
    errors      : utils.generateDataLegend(['iterations', 'error'])
  };
  ...
  for (name in results) {
    if (this.aspects.hasOwnProperty(name)) {
      ...
      legend.unshift('x');
      data[name] = utils.generateDataLegend(legend) + data[name];
    }
  }
