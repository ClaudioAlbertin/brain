...
Network.prototype = new ReportEmitter();
...
Network.prototype.saveJSON = function (path) {
  return fs.writeFileSync(path, JSON.stringify(this.toJSON()));
};
...
Network.prototype.propagate = function (inputs) {
  ...
  this.report('finished-execution', {
    iterations : i
  });
...
Network.prototype.clone = function () {
  ...
  return new Network(_.clone(this.layers), utils.cloneWeights(this.weights));
};
